...
void bfs(map<string, vector<string>>& tree, vector<Node>& distances);

int year2024_day6_puzzle1() {
    ifstream f("ressources/Year2024/day6/part1.txt");
...
void bfs(map<string, vector<string>>& tree, vector<Node>& distances) {
    ...
    while (!file.empty()) {
        ...
        for (const auto& v : tree[name]) {
            file.emplace(v, distance + 1, path + v);
        }
    }
}