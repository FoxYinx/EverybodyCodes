...
void bfsPart3(map<string, vector<string>>& tree, vector<Node>& distances);

int year2024_day6_puzzle3() {
    ifstream f("ressources/Year2024/day6/part3.txt");
    ...
    while (getline(f, s)) {
        ...
        if (key == "BUG" || key == "ANT") continue;
        vector<string> values;
        while (regex_search(s, sm, regexp2)) {
            if (sm[1] != "ANT" || sm[1] != "BUG") values.push_back(sm[1]);
            s = sm.suffix();
        }
        if (!values.empty()) tree[key] = values;
    }

    vector<Node> distances;
    bfsPart3(tree, distances);
